...
For example, you can generate key pairs using a `node-template` subcommand, the standalone [Subkey](/v3/tools/subkey) command-line program, the Polkadot-JS application, or third-party key generation utilities.
...
You have already used some command-line options to start your local blockchain node using the predefined `alice` and `bob` accounts.
