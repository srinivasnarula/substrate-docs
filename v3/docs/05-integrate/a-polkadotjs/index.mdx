...
<Message
  type={`gray`}
  title={`Note`}
  text={`While the project is named after "Polkadot", know that these tools, interfaces, and
...
<RelatedMaterialBlock
  title={``}
  text={`The API provides application developers the ability to query a node and interact with the Polkadot
