...
Using a child digital tree, or trie, provides two advantages over using standard storage. First, it allows you to remove the entire trie in a single storage write when the fund is dispensed or dissolved. Second, it allows any contributor to prove that they contributed using a Merkle Proof.

In this guide, you'll see how to use one trie for each active crowdfund. Any user can start a crowdfund by
specifying a goal amount for the crowdfund, an end time, and a beneficiary who will receive the pooled funds if the goal is reached by the end time. If the fund is not successful, it enters into a
retirement period when contributors can reclaim their pledged funds. An unsuccessful fund can be dissolved, sending any remaining tokens to the user who dissolves it.
...
1. Declare your pallet's configuration traits.
  ...
  Add the traits to the list of types.
...
1. Create a custom metadata struct.
...
1. Declare your storage items.
  ...
  Use those types to create one `StorageMap` item that tracks the funds by index and a second item, `StorageValue`, that keeps track of `FundIndex`:
